...
from ..stage_two.models import RealizedBeat, VoiceStageArtifact, WordTimestamp
from .models import (
    CaptionCue,
    DerivedAsset,
    OverlayCue,
...
class TimelineCompositionPipeline:
    ...
    def run(
        ...
        if not (
            ...
            == stage_three.project_id
            == stage_four.project_id
        ):
            raise ValueError("stage project_id values do not match")

        assets = {asset.asset_id: asset for asset in stage_four.assets}
        resolutions = {
            item.visual_request_id: item for item in stage_four.resolutions
        }
        visuals_by_beat = {
            item.beat_id: item
            for item in stage_three.editorial_plan.visual_requirements
        }
        claims_by_beat: dict[str, list[str]] = {}
        for claim in stage_three.editorial_plan.claims:
        ...
        beats = stage_two.realized_beats
        ...
        for index, beat in enumerate(beats):
            ...
            clip_end = (
                ...
                else stage_two.timed_audio.duration_ms
            )
            playback_path = (
                derivative.local_path if derivative else asset.local_path
            )
            is_video = derivative is not None or asset.modality == "ai_video"
        ...
        captions = _build_captions(
            beats,
            stage_two.word_alignment.words,
        )
        timeline = TimelinePlan(
            audio_file=stage_two.timed_audio.audio_file,
...
def _evaluate(
    ...
    for clip in timeline.clips:
        path = project_dir / clip.playback_path
        if not path.is_file() or path.stat().st_size == 0:
            missing += 1
            issues.append(f"{clip.clip_id} playback asset is missing")
    full_coverage = (
        timeline.clips[0].timeline_start_ms == 0
        and timeline.clips[-1].timeline_end_ms == timeline.duration_ms
        and all(
            left.timeline_end_ms == right.timeline_start_ms
            for left, right in zip(timeline.clips, timeline.clips[1:])
        )
    )
    if len(timeline.clips) != len(beats):
        issues.append("clip count does not match realized beat count")
    if not full_coverage:
        issues.append("clips do not cover the complete audio timeline")
    return TimelineQuality(
        ...
        screen_derivative_count=len(derivatives),
        issues=issues,
    )
